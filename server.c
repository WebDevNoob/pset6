...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            while (sp != NULL){
                switch (i){
                    ...
                    case 1:
                        ...
                        fileType = (strchr( sp, '.')) + 1;
                        
                        for(int k = 0; k < strlen(sp); k++){
                            tmpPath[k] = sp[k];
                        }
                        
                        if( (strchr(sp,'?') != NULL) ){
                            queryCheck = (strchr(sp, '?')) + 1;
                        }else{
                            queryCheck = "";
                        }
                        break;
            ...
            char* query = queryCheck;

            // TODO: concatenate root and absolute-path
            
            char path[50] = ""; 
            sprintf(path,"%s%s%s", root, tmpPath, queryCheck);
